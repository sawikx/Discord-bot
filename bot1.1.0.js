...
const command = args.shift().toLowerCase();
 ...
 switch (command){
  ...
  break;
  case 'role': //users can add what role they want
    let[role,whatitdoes]=args;
    var memberadd =  message.member;
    switch(role){
      case "your role":
        var rolaadd = message.guild.roles.find(r => r.id === "your role id"); //I prefer id but you can use name : r.name
        if(whatitdoes == "delete"){
          memberadd.removeRole(rolaadd);
        }
  ...
  case 'help'://information about commands
          var he1="```";
          var css="fix"
          message.channel.send(he1+css+enter+'!command - options - what it does'+enter+""+enter+'!8ball - - answer your question'+enter+'!los - number of max, number of min (draw range) - creates random numbers'+enter+'!wyb (a b ...)- ten items max - choose one of the items'+enter+'!licz (44 + 44) -| + add | - subtracts | * multiplies | / divides | v square root | ^ power | - calculator '+enter+"!role {name role} {add or delete}"+he1);
  break;
   }
});
